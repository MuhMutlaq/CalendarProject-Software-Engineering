...
import os
UPL_DIR= os.path.join(os.path.dirname(__file__), "uploads")
...
def ocr(image_path: str):
    try:
        text= reader.readtext(image_path, detail= 1, paragraph= False, decoder= "beamsearch")
        return text
...
# coursecode_pattern= re.compile(r"[A-Z]{2, 10}\d{3}")
# duration_pattern= re.compile(r"\d{1,2}:\d{2}\sto\s\d{1,2}:\d{2}")
#!/usr/bin/env python3
"""
